...
export function getRandomIntro(featureKey = 'menu', user = '', botName = 'MARKONAH STORE') {
    ...
    // Jika argumen pertama berupa tag user (misal '@62858...' atau nomor user)
    if (typeof featureKey === 'string' && (featureKey.startsWith('@') || /^\d+$/.test(featureKey.replace(/[^0-9]/g, '')) && !defaultIntros[featureKey])) {
        key = 'menu';
        targetUser = featureKey;
        if (user) bName = user;
    }

    const data = loadJson(msgAtasPath, defaultIntros);
    const list = data[key] || data['menu'] || defaultIntros.menu;
    const template = list[Math.floor(Math.random() * list.length)];
    return template.replace(/\{user\}|@user/g, targetUser).replace(/\{botName\}/g, bName);
}
