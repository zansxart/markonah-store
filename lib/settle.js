...
export async function settlePaid(conn, invoiceId, { source = 'manual' } = {}) {
    ...
    if (trx.trx_type === 'smm') {
        // Idempotency SMM: kalau order sudah pernah dikirim ke provider
        // (smm_order_id terisi), jangan tembak createOrder lagi — cukup
        // tandai selesai. Cegah order dobel saat `acc`/`proses` diulang.
        if (trx.smm_order_id) {
            storeDB.completeTransaction(invoiceId, [`Order ID Sosmed: ${trx.smm_order_id}`]);
            return { ok: true, status: 'done', trx, smmOrderId: trx.smm_order_id, source };
        }
