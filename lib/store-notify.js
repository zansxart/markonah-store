...
export function notifyNewOrder(conn, { invoiceId, buyerJid, productName, total, chatJid }) {
    ...
    let ownerText = `🔔 *PESANAN BARU — PERLU KONFIRMASI*\n`;
    ownerText += `Invoice: ${copyable(invoiceId)}\n`;
    ownerText += `Pembeli: @${String(buyerJid).split('@')[0]}\n`;
    ownerText += `Produk: ${productName}\n`;
    ownerText += `Total: Rp ${rp(total)}\n`;
    ownerText += `Status: Pending\n\n`;
    ownerText += `Setelah pembayaran masuk, konfirmasi dengan:\n${copyable(`${prefix}acc ${invoiceId}`)}`;
