...
import { uploadFile, uploadImage } from './uploader.js'
...
const responseToBuffer = async (response) => Buffer.from(await response.arrayBuffer())
/**
 * Image to Sticker
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (url) {
        ...
        img = await responseToBuffer(res)
      }
...
async function canvas(code, type = 'png', quality = 0.92) {
  ...
  let image = await responseToBuffer(res)
  return image
}
...
async function sticker3(img, url, packname, author) {
  ...
  return await responseToBuffer(res)
}
...
function sticker6(img, url) {
  return new Promise(async (resolve, reject) => {
    ...
    const tmp = path.join(process.cwd(), 'tmp', `${+new Date()}.${type.ext}`)
    ...
    Fffmpeg
      .on('error', function (err) {
        ...
        reject(new Error('Media tidak valid atau format tidak didukung untuk sticker.'))
      })
...
async function sticker(img, url, ...args) {
  ...
  console.error('[STICKER ERROR]', lastError?.message || lastError)
  return lastError
}
